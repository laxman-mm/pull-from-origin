...
export default function Auth() {
  ...
  useEffect(() => {
    if (user) {
      // Check if this is a verification success
  ...
  }, [user, isAdmin, navigate, searchParams, from]);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!result.success) {
        setErrors({ submit: result.error || 'Failed to sign in with Google' });
      }
    } catch (error) {
      setErrors({ submit: 'An unexpected error occurred' });
    } finally {
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              {isSignUp 
                ? 'Join Sikkolu Ruchulu to save your favorite recipes and more'
