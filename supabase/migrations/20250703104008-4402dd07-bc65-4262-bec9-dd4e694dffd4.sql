...
-- Update the search_recipes function to handle tag-based filtering
DROP FUNCTION IF EXISTS public.search_recipes(text, text, text, integer, integer);
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(f.url, '')::character varying as image_url,
    COALESCE(
      jsonb_build_object(
        'id', a.id,
        'name', a.name,
  ...
  LEFT JOIN receipes_tags_lnk rtl ON r.id = rtl.receipe_id
  LEFT JOIN tags t ON rtl.tag_id = t.id AND t.published_at IS NOT NULL
  ...
  WHERE 
    ...
    AND (tag_filter IS NULL OR EXISTS (
      SELECT 1 FROM receipes_tags_lnk rtl2 
      JOIN tags t2 ON rtl2.tag_id = t2.id 
      WHERE rtl2.receipe_id = r.id AND t2.name = tag_filter AND t2.published_at IS NOT NULL
    ))
